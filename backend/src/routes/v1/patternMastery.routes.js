...
}), 'query'), cache(60, { privacy: 'private', maxAge: 60, keyPrefix: 'pattern-mastery:list' }), patternMasteryController.getPatternMasteryList);

router.get('/stats', auth, rateLimiters.userLimiter, cache(60, { privacy: 'private', maxAge: 60, keyPrefix: 'pattern-mastery:stats' }), patternMasteryController.getPatternStats);
...
}), 'query'), cache(300, { privacy: 'private', maxAge: 300, keyPrefix: 'pattern-mastery:recommendations' }), patternMasteryController.getRecommendations);
...
}), 'query'), cache(60, { privacy: 'private', maxAge: 60, keyPrefix: 'pattern-mastery:weakest' }), patternMasteryController.getWeakestPatterns);

router.get('/strongest', auth, rateLimiters.userLimiter, validate(Joi.object({
  limit: Joi.number().integer().min(1).max(20).default(5),
  metric: Joi.string().valid('confidence', 'masteryRate', 'lastPracticed').default('confidence')
}), 'query'), cache(60, { privacy: 'private', maxAge: 60, keyPrefix: 'pattern-mastery:strongest' }), patternMasteryController.getStrongestPatterns);
...
}), 'query'), cache(300, { privacy: 'private', maxAge: 300, keyPrefix: 'pattern-mastery:progress' }), patternMasteryController.getPatternProgress);

router.get('/:patternName', auth, rateLimiters.userLimiter, validate(Joi.object({
  patternName: Joi.string().trim().required()
}), 'params'), cache(30, { privacy: 'private', maxAge: 30, keyPrefix: 'pattern-mastery:pattern' }), patternMasteryController.getPatternMastery);
